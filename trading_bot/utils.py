...
def get_insider_trade_data(acq_file, dis_file):
    """Reads insider trades from csv file
    """
    #return only the days 
    df_acq = pd.read_csv(acq_file, header=None).values.flatten()
    df_dis = pd.read_csv(dis_file, header=None).values.flatten()
